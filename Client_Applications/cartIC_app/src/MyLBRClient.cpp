...
MyLBRClient::MyLBRClient(double freqHz, double amplitude){
    ...
    myIC_p = new CartImpController( 'T', myLBR->nq  );

    K_p = Eigen::Matrix3d::Identity( 3, 3 );
    K_p = 1000 * K_p;
    myIC_p->setStiffness( K_p );
    myIC_p->setDampingRatio( 0.7 );

    // Rotational Impedance Controller
    myIC_r = new CartImpController( 'R', myLBR->nq  );

    K_r = Eigen::Matrix3d::Identity( 3, 3 );
    K_r = 100 * K_r;
    myIC_r->setStiffness( K_r );
    myIC_r->setDampingRatio( 0.6 );
...
void MyLBRClient::command()
{
    ...
    if(currentTime < sampleTime)
    {
        ...
        Eigen::MatrixXd H_ini = myLBR->getForwardKinematics( q );
        Eigen::Matrix3d R_ini = H_ini.block< 3, 3 >( 0, 0 );
        Eigen::Vector3d p_ini = H_ini.block< 3, 1 >( 0, 3 );
        myIC_p->setZFT( p_ini );
        myIC_r->setZFT( R_ini );
    }
    ...
    myIC_p->setKinematics( dq, H, J, M );
    myIC_r->setKinematics( dq, H, J, M );

    // Control wrenches for translational IC
    Eigen::VectorXd F_el_p = myIC_p->getElasticWrench();
    Eigen::VectorXd F_d_p = myIC_p->getDampingWrench();

    // Control torque
    Eigen::VectorXd tau_el_p = J.transpose() * ( F_el_p - F_d_p );

    // Control wrenches for rotational IC
    Eigen::VectorXd F_el_r = myIC_r->getElasticWrench();

    Eigen::VectorXd F_d_r = myIC_r->getDampingWrench();

    // Control torque
    Eigen::VectorXd tau_el_r = J.transpose() * ( F_el_r - F_d_r );

    // Hold initial translation and rotation
    tau_motion = tau_el_p + tau_el_r;
